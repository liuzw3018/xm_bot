...
// @return:       nil
func EventHandle(msg map[string]interface{}) {
	//fmt.Println(msg)
	// TODO 匹配事件类型
	...
	case "notice":
		logMsg := fmt.Sprintf("群聊 %v 接收到来自 %s 的 %s 消息", msg["group_id"], msg["user_id"], msg["notice_type"])
		log.Printf(logMsg)
		global.BotInfoLogMsgChan <- logMsg
	}
}
...
// @description:  处理消息中包含的CQ码消息，并判断是否at了机器人
// @auth:         liuzw3018
